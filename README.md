...
Надёжность у современных дисков так себе, особенно если речь идёт про хранилище важных данных, поэтому следует периодически проверять их состояние. Кроме того, рекомендуется сделать проверку сразу после покупки накопителя, особенно если ты таки решил взять HDD с рук. Как это делать:

##### Оценка состояния по SMART

Всё очень просто: качаешь наиболее подходящую тебе утилиту, делаешь опрос SMART-таблицы накопителя, смотришь результат. Самые важные показатели:
...
- C5 - Current Pending Sector Count - количество секторов под вопросом. Тут одно из двух: или они в будущем пойдут в 05, или в процессе изсправятся и просто покинут этот список. Как правило этот показатель говорит о наличии программных бэд-секторах, часто появляющихся при отключении света во время записи на диск. С большой степенью вероятности исправятся путём банальной перезаписи.
- С6 - Uncorrectable Sector Count - никогда не видел, чтобы он где-то был задействован.
...
- С2 - Temperature - температура накопителя, собственно. Как правило, сам регистр показывает погоду на Марсе, но любой софт это дело парсит и выводит значение температуры отдельно. Есть не слишком свежий [статистический график](https://en.wikibooks.org/wiki/Minimizing_Hard_Disk_Drive_Failure_and_Data_Loss/Environmental_Control), можешь ориентироваться по нему. Врядли у тебя дома температуры ниже минимально комфортной для человека без шубы, посему главная задача - не перегревать накопитель. Оптимальная температура - 35-40 градусов, но и от 30 градусов никто не умрёт, а вот циклический прогрев накопителя до 50 градусов может негативно сказаться на его самочувствии.
- 09 - Power-On Hours - время наработки диска. Статистический параметр, у новья он должен стремиться к нули, но пара-тройка часов работы - нормальное дело, диск проходил тестирование у производителя. Практически любой диск способен отработать 20...50 и больше часов, были замечены долгожители с наработкой более 100к часов.
